<template>
  <div class="wrappers">
    <div class="list-header">
      <div class="list-header-text">{{isSelf?'我': authorname}}的收藏</div>
      <div class="sorts" v-if="isSelf">
        <el-button type="text" icon="el-icon-plus">新建收藏夹</el-button>
      </div>
    </div>
    <div class="list" v-if="datalist && datalist.length">
      <div class="list-item" v-for="item in datalist" :key="item.id">
        ...
        <user-avatar :userInfo="item.user[0]" avatarRound="avatar-round" avatarSize="avatar-small"></user-avatar>
        <!-- content -->
        <div class="content">
          <!-- 简短展示内容，去掉标签 -->
